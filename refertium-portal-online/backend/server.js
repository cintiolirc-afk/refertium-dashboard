...
async function serveUserApp(req, res, db, user, targetUserId) {
  ...
  // `{userId}-generated-{timestamp}.html`), rigeneriamo on-the-fly dal
  // template madre più recente, così le modifiche al template (es. prompt
  // dettatura, fix UX) si propagano automaticamente senza richiedere all'admin
  // di cliccare "Genera HTML" su ogni medico dopo ogni deploy.
  // Gli HTML caricati manualmente dall'admin (filename diverso) restano intatti.
  ...
  if (isAutoGenerated) {
    try {
      ...
      // Fallback al file su disk se la rigenerazione fallisce
